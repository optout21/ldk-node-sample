...
use ldk_node::{Builder, LogLevel, Node, UserChannelId};
use std::env;
use std::io;
use std::io::Write;
use std::ops::Add;
...
fn open_channel(node: &Node, node_id: PublicKey, peer_addr: SocketAddress, chan_amt_sat: u64) {
	...
	match node.open_channel(node_id, peer_addr, chan_amt_sat, None, None) {
...
fn get_balances(node: &Node) {
	...
	let ln_total = balances.total_lightning_balance_sats;
	let channels = node.list_channels();
	let ln_spendable =
...
fn node_info(node: &Node) {
	...
	let local_spendable_msat = channels.iter().map(|c| c.outbound_capacity_msat).sum::<u64>();
	println!("Local balance (msat):   \t{}", local_spendable_msat);
	let peers = node.list_peers();
			...
			match word {
				...
				"closechannel" => {
					...
					if user_channel_id_str.is_none() {
						println!("ERROR: closechannel requires a UserChannelID: `closechannel <user_channel_id> <peer_pubkey>`");
						continue;
					}
					let user_channel_id = match user_channel_id_str.unwrap().parse() {
						Err(_) => {
							println!("Invalid UserChannelID {}", user_channel_id_str.unwrap());
							continue;
						}
						Ok(ci) => UserChannelId(ci),
					};

					let peer_pubkey_str = words.next();
					if peer_pubkey_str.is_none() {
						println!("ERROR: closechannel requires a UserChanneID: `closechannel <user_channel_id> <peer_pubkey>`");
						continue;
					}
...
fn main() {
	...
	builder.set_chain_source_esplora(settings.esplora_url, None);
